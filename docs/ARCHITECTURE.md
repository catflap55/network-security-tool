...
- **Double-click** **[Windows/Start.cmd](../Windows/Start.cmd)** (uses `cmd.exe`). It opens a **second** window running `npm run dev:full`, waits for the API and Vite, then opens **http://127.0.0.1:5173/**.
- **If you already use PowerShell:** from the **Windows** folder run **`.\Start.ps1`** (not the `.cmd` contents — those are batch syntax and will error if pasted into PowerShell).
- **Stop:** double-click **[Windows/Stop.cmd](../Windows/Stop.cmd)**, or in PowerShell **`.\Stop.ps1`**. Either frees ports **5173** and **8000**. You can also close the **second** window running `npm run dev:full`.
...
2. Backend: from the **repo root**:
   ...
   - **Windows:** `.\Windows\start-backend.ps1` (also installs requirements then starts Uvicorn).
